...
//! prebuilt Deepgram component and drives the `/v1` contract in-process, exactly
...
//!
//! Tests pair this with a `wiremock` mock of the *Deepgram upstream*, so both
...
pub fn component_path() -> Option<PathBuf> {
    let p = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("target/wasm32-wasip2/release/super_stt_backend_deepgram.wasm");
...
/// A loaded Deepgram component, driven over the batch `/v1` contract.
...
impl WasmBackend {
    ...
    /// Like [`Self::transcribe_audio`], but includes a `language` field in the
    /// request body — exercises the component's language forwarding (e.g. the
    /// reserved `auto` → Deepgram `multi` mapping).
    ///
    /// # Errors
    /// Returns an error if the component cannot be invoked or the backend
    /// reports a non-success status.
