...
async fn ping_and_status() {
    ...
    let backend = WasmBackend::new(&path, Vec::new(), "nova-3".to_string(), Vec::new())
        .expect("load backend");
...
async fn transcribe_without_key_reports_missing_secret() {
    ...
    let mut backend = WasmBackend::new(&path, Vec::new(), "nova-3".to_string(), Vec::new())
        .expect("load backend");
    ...
    assert!(
        err.to_string().contains("Deepgram API key not set"),
