...
//! Optional live test against the real Deepgram API. Transcribes a real WAV.
//! Gated — self-skips unless explicitly enabled, so it never runs in hosted CI.
//!
//! Enable with:
//!   SUPER_STT_TEST_DEEPGRAM=1 DEEPGRAM_API_KEY=... \
//!   SUPER_STT_TEST_AUDIO=/path/to/mono.wav \
//!   just build-component && cargo test --test live_deepgram -- --nocapture
...
async fn live_deepgram() {
    if std::env::var("SUPER_STT_TEST_DEEPGRAM").is_err() {
        return;
    }
    ...
    let key =
        std::env::var("DEEPGRAM_API_KEY").expect("DEEPGRAM_API_KEY must be set for the live test");
    let audio_path =
